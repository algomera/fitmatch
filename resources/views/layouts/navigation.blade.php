    ...
    <div class="mx-auto max-w-7xl px-2 sm:px-4 lg:px-8">
        <div class="flex h-16 justify-between">
            ...
            <div class="flex items-center lg:hidden">
                <!-- Mobile menu button -->
                <button x-cloak x-on:click="open = !open" type="button" class="inline-flex items-center justify-center rounded-md p-2 text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-indigo-500" aria-controls="mobile-menu" aria-expanded="false">
                    <span class="sr-only">Open main menu</span>
                    <svg x-cloak :class="{'block': !open, 'hidden': open}" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
                            ...
                            <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 bg-white hover:text-gray-700 focus:outline-none transition ease-in-out duration-150">
                                <div>
                                    @if(auth()->user()->informations->profile_image)
                                        <img class="h-8 w-8 rounded-full" src="{{ asset(auth()->user()->informations->profile_image) }}" alt="">
                                    @else
        ...
        <div class="border-t border-gray-200 pb-3 pt-4">
            <div class="flex items-center px-4">
                <div class="flex-shrink-0">
                    @if(auth()->user()->informations->profile_image)
                        <img class="h-10 w-10 rounded-full" src="{{ asset(auth()->user()->informations->profile_image) }}" alt="">
                    @else
