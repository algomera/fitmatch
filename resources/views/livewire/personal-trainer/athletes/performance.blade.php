<div>
    ...
    <div class="relative min-h-screen bg-fit-lighter-gray overflow-x-scroll px-4 py-4">
        @forelse($weeks as $week)
            @if(!$selectedWeek)
                <h3 class="text-lg font-semibold">Settimana {{ $week->week }}</h3>
                @foreach($week->workout_days as $day)
                    @if(!$selectedDay)
                        <h3 class="text-sm font-semibold">{{ config('fitmatch.days.' . $day->day) }}</h3>
                        <div class="flex items-center">
                            @foreach($day->workout_sets as $set)
                                <div class="flex flex-col w-full z-[50] space-y-3">
                                    @foreach($set->workout_series as $serie)
                                        <div class="flex">
                                            <div wire:key="set-{{$week->id}}-{{$day->id}}{{$set->id}}-{{$serie->id}}"
                                                 class="flex">
                                                <div class="sticky left-0 z-[49] flex min-h-[10rem] px-4">
                                                    <div class="relative flex flex-col items-end px-2 w-8">
                                                    <span class="text-xl font-bold text-fit-magenta">
